...
md_absentee_ballots <- md_absentee_ballots |>
  mutate(pct_dem = (dem_received / total_received)) |>
  mutate(pct_rep = (rep_received / total_received)) |>
  mutate(pct_diff = pct_dem - pct_rep)|>
  arrange(pct_dem)|>
  head(10)

ggplot(data = top10_counties) +
  geom_bar(aes(x=reorder(county_name,pct_dem), weight= pct_diff)) +
  coord_flip() + 
  labs(
    title="Top 10 Counties with Lowest Perc of Dem Ballots ",
    x = "County",
    y = "Percentage of returned ballots",
    caption = "source: Maryland State Board of Elections")
...
md_absentee_ballots |>
  ...
  geom_text_repel(aes(x = rep_sent, y = rep_received, label = county_name),
                  color = "black", size=2)+
  geom_smooth(aes(x = oth_sent, y = oth_received, label = county_name),
                  color = "black", size=2)+
  geom_smooth(aes(x = dem_sent, y = dem_received), method = lm, se= FALSE, color= "green")+
  geom_smooth(aes(x = rep_sent, y = rep_received), method = lm, se= FALSE, color= "orange")+
  geom_smooth(aes(x = oth_sent, y = oth_received), method = lm, se= FALSE, color= "blue")+
  
  labs( 
    title="Absentee Ballot per County",
    x = "Sent"
    y = "Received",
    colour = "Key",
...
```
*I AM UNSURE ABOUT THE ERROR AND WHY THE SCATTER PLOT WILL NOT LOAD*
